...
**For the enablement lead running this session:**

### Before the Workshop
- Ensure participants have Claude Code CLI installed and authenticated
- Ensure participants have GitHub Copilot extension active in VS Code/IntelliJ
- Have a sample Java/Spring Boot repository ready as a fallback demo

### During the Workshop
1. Participants `cd` into their own project
2. Paste the prompt above into `claude --plan`
3. Review the plan Claude Code proposes — this is the teachable moment
4. Approve the plan and let it execute
5. Review the generated files together as a group
6. Discuss what to keep, what to change, what's missing

### Key Discussion Points
- "What did the AI get wrong about your project?" → builds critical review habit
- "What did it surface that you'd forgotten to document?" → demonstrates value
- "What would a new team member need that's still missing?" → identifies gaps
- "Does the project use a design system? If GDS, are govuk-* components used correctly and accessibly?"
- "Are GOV.UK Design Principles (progressive enhancement, plain language, start with user needs) encoded anywhere in the project today?"
- The instruction files are living documents — update them as the project evolves

### The 80% Conversation
Most workshops stop at "it helps you write code faster." Push further:
- How many hours/week do you spend on PR reviews? Can these rules speed that up?
- How often do you write ADRs? Would a `/project:write-adr` command lower the friction?
- When was the last time you updated your README? Could the AI do the first draft?
